...
- offline fallback для GitHub Pages;
...
Поведение:
- **backend доступен** → сайт использует live API, отдельное предупреждение не показывается;
- **backend недоступен** → сверху появляется сообщение  
  **«Сервер с актуальными данными сейчас не подключен»**,  
  а интерфейс автоматически использует `docs/fallback-data.json`.
