...
def init():
    ...
    for s in data["sources"]:
        cur.execute("INSERT INTO sources(key,name,type,url,retrieved_at,notes) VALUES(?,?,?,?,?,?)",
                    (s["key"],s["name"],s["type"],s["url"],s["retrieved_at"],s.get("notes")))
    for p in data["programs"]:
        cur.execute(
            ...
            (p["slug"],p["code"],p["title"],p["level"],p.get("faculty"),p.get("form"),p.get("duration"),
             ...
             p.get("master_code"),p.get("master_url"),data["retrieved_at"])
        )
    for p in data["professions"]:
        cur.execute(
            ...
            (p["slug"],p["name"],p["sector"],p["program_slug"],p.get("description"),dumps(p["skills"]),dumps(p["riasec"]),
             dumps(p["values"]),dumps(p["subjects"]),p["market_key"])
        )
        pr=next(x for x in data["programs"] if x["slug"]==p["program_slug"])
        evidence=(pr.get("disciplines") or [])[:5]
        cur.execute(
            "INSERT INTO matches(profession_slug,program_slug,status,score,evidence_json,verified_at) VALUES(?,?,?,?,?,?)",
            (p["slug"],p["program_slug"],"approved",88,dumps(evidence),data["retrieved_at"])
        )
    for slug,m in data["market"].items():
        cur.execute(
            ...
            (slug,m["source_name"],m["source_url"],m["geography"],m["snapshot"],m.get("median_salary"),
             m.get("salary_min"),m.get("salary_max"),m.get("salary_vacancies"),m.get("vacancies_total"),
             m.get("yoy_salary"),dumps(m.get("bands",{})),m["quality"],m.get("note"))
        )
    for slug,branches in data["career"].items():
        for branch,nodes in branches.items():
            cur.execute("INSERT INTO career_paths(profession_slug,branch,nodes_json) VALUES(?,?,?)",(slug,branch,dumps(nodes)))
    con.commit()
    con.close()
    print(DB)
