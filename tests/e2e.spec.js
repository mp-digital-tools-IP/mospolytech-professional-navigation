...
async function expectImageLoaded(locator) {
  ...
  const ok = await locator.evaluate(img => img.complete && img.naturalWidth > 0 && img.naturalHeight > 0);
  expect(ok).toBeTruthy();
}

test.describe('desktop smoke', () => {
  test.skip(({ project }) => !project.name.startsWith('desktop'));
  ...
  test('основная навигация действительно переключает разделы', async ({ page }) => {
    await boot(page);
    const ids = ['diagnostics','results','recommendations','atlas','programs','trajectory','prep','dpo','market','profile','help'];
    for (const id of ids) {
...
test.describe('mobile smoke', () => {
  test.skip(({ project }) => !project.name.startsWith('mobile'));
  ...
  test('нижняя мобильная навигация работает', async ({ page }) => {
    await boot(page);
    const ids = ['diagnostics','recommendations','atlas','trajectory','home'];
    for (const id of ids) {
      await page.locator('.mobile-bottom-nav button[data-mobile-view="'+id+'"]').click();
      await expect(page.locator('#'+id)).toHaveClass(/active/);
      await expect(page.locator('#'+id)).toBeVisible();
    }
  });
